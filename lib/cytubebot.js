...
var bean_tube = 'pienudes_playlist';
var bean = new fivebeans.client();
...
CytubeBot.prototype.handleChangeMedia = function(data) {
	...
	var media = utils.handle(this, "findIndexOfVideoFromVideo", {media: this.currentMedia});
	if (typeof media[0].uid !== "undefined") {
		var vid = utils.handle(this, "getVideoFromUID", media[0].uid);
		if (vid) {
			bean.use(bean_tube, function(err) {
				if (err) {
					console.log(err);
				} else {
					bean.put(1, 0, 60, JSON.stringify(vid), function(err, jobid) {
						if (err) {
							console.log(err);
						}
					});
